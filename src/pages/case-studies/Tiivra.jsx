...
import { tiivra } from '../../data/tiivra'
...
export default function Tiivra() {
  const study = tiivra
  const mid = study.iaNodes.slice(1, 5)
  const home = study.iaNodes[0]
  const pdp = study.iaNodes[5]
          ...
          <div className="wrap">
            ...
            <div className="cs-ia">
              <div className="cs-sitemap">
                <span className="cs-node accent">{home.label}</span>
                <span className="cs-line" />
                <div className="cs-sitemap-row">
                  {mid.map((n) => (
                    <span className="cs-node" key={n.label}>{n.label}</span>
                  ))}
                </div>
                <span className="cs-line" />
                <span className="cs-node">{pdp.label}</span>
              </div>
              <div className="cs-ia-copy">
                {study.iaGroups.map((g) => (
                  <div key={g.title}>
                    <h3>{g.title}</h3>
                    <p>{g.body}</p>
                  </div>
        ...
        <Reveal>
          <div className="wrap">
            ...
            <div className="cs-grid-3">
              ...
              <div className="cs-card surface">
                <p className="cs-kicker">Imagery Direction</p>
                <p>{study.imagery}</p>
              </div>
            </div>
          </div>
